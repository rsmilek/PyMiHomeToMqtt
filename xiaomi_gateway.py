...
class MyLogger:
    def info(self, arg1, *argv):
        print('INFO', arg1, *argv)

    def error(self, arg1, *argv):
        print('ERROR', arg1, *argv)

    def debug(self, arg1, *argv):
        print('DEBUG', arg1, *argv)
        ...
        try:
            ...
            while True:
                ...
                if disabled:
                    ...
                    _LOGGER.info(
                        'Xiaomi Gateway %s found at IP %s', sid, ip_add)
    ...
    def _create_mcast_socket(self):
        ...
        if self._interface != 'any':
            ...
            mreq = socket.inet_aton(
                self.MULTICAST_ADDRESS) + socket.inet_aton(self._interface)
        else:
            if platform.system() != "Windows":
                sock.bind((self.MULTICAST_ADDRESS, self.MULTICAST_PORT))
            else:
...
class XiaomiGateway:
    ...
    def __init__(self, ip_adress, port, sid, key, discovery_retries, interface, proto=None):
        ...
        if proto is None:
            ...
            proto = _get_value(
                resp, "proto_version") if _validate_data(resp) else None
    ...
    def _discover_devices(self):
        ...
        cmd = '{"cmd" : "get_id_list"}' if int(
            self.proto[0:1]) == 1 else '{"cmd":"discovery"}'
        ...
        for sid in sids:
            ...
            for retry in range(self._discovery_retries):
                _LOGGER.debug("Discovery attempt %d/%d",
                              retry + 1, self._discovery_retries)
                resp = self._send_cmd(cmd, "read_ack") if int(
                    self.proto[0:1]) == 1 else self._send_cmd(cmd, "read_rsp")
    ...
    def write_to_hub(self, sid, **kwargs):
        ...
        if not self.token:
            _LOGGER.debug(
                'Gateway Token was not obtained yet. Cannot send commands to the gateway.')
            return False
        ...
        if resp is None or "token" not in resp:
            _LOGGER.error(
                'No new token from gateway. Can not send commands to the gateway.')
            return False
        self.token = resp['token']
        if int(self.proto[0:1]) == 1:
            data['key'] = self._get_key()
    ...
    def push_data(self, data):
        ...
        if not _validate_data(data):
            return False
        jdata = json.loads(data['data']) if int(
            self.proto[0:1]) == 1 else _list2map(data['params'])
        if jdata is None:
            return False
        sid = data['sid']
